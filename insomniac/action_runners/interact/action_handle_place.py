...
def extract_place_instructions(source):
    ...
    if selected_instruction is None:
        print(
            "There is no special interaction-instructions for "
            + source
            + ". Working with "
            + source
            + " recent-likers."
...
def handle_place(
    ...
    if (
        instructions == PlaceInteractionType.RECENT_LIKERS
        or instructions == PlaceInteractionType.RECENT_POSTS
    ):
    ...
    def interact_with_profile(liker_username, liker_username_view):
        ...
        """
        (
            is_interact_limit_reached,
            interact_reached_source_limit,
            interact_reached_session_limit,
        ) = is_limit_reached(
            InteractAction(
                source_name=place,
                source_type=source_type,
        ...
        (
            is_get_profile_limit_reached,
            get_profile_reached_source_limit,
        ...
        if is_passed_filters is not None:
            if not is_all_filters_satisfied:
                should_continue, _ = is_passed_filters(
                    device, liker_username, reset=False
                )
                if not should_continue:
        ...
        (
            is_like_limit_reached,
            like_reached_source_limit,
            like_reached_session_limit,
        ) = is_limit_reached(
            LikeAction(source_name=place, source_type=source_type, user=liker_username),
            session_state,
        )

        (
            is_follow_limit_reached,
            follow_reached_source_limit,
            follow_reached_session_limit,
        ) = is_limit_reached(
            FollowAction(
                source_name=place, source_type=source_type, user=liker_username
            ),
            session_state,
        )

        (
            is_watch_limit_reached,
            watch_reached_source_limit,
        ...
        (
            is_comment_limit_reached,
            comment_reached_source_limit,
            comment_reached_session_limit,
        ) = is_limit_reached(
            CommentAction(
                source_name=place,
                source_type=source_type,
        ...
        can_follow = (
            ...
            and storage.get_following_status(liker_username) == FollowingStatus.NONE
            and follow_percentage > 0
        )
        can_watch = (
            (not is_watch_limit_reached) and do_have_stories and stories_value > 0
        )
        can_comment = (
            (not is_comment_limit_reached) and not is_private and comment_percentage > 0
        )
        ...
        if not can_interact:
            ...
            print_interaction_types(
                liker_username, can_like, can_follow, can_watch, can_comment
            )
            ...
            is_liked, is_followed, is_watch, is_commented = interaction(
                username=liker_username, interaction_strategy=interaction_strategy
            )
        ...
        if (
            ...
            if (
                like_reached_source_limit is not None
                and like_reached_session_limit is None
            ) or (
                follow_reached_source_limit is not None
                and follow_reached_session_limit is None
            ):
                can_continue = False
            ...
            if (
                (like_reached_session_limit is not None and is_likes_enabled)
                or not is_likes_enabled
            ) and (
                (follow_reached_session_limit is not None and is_follow_enabled)
                or not is_follow_enabled
            ):
                can_continue = False
    ...
    def interact_with_feed_post(posts_views_list):
        (
            ...
            InteractAction(
                source_name=place, source_type=source_type, user=None, succeed=True
            ),
            session_state,
        )
        ...
        (
            ...
            LikeAction(source_name=place, source_type=source_type, user=None),
            session_state,
        )
        ...
        if like_chance > like_percentage:
            ...
            if author_name is not None:
                on_action(
                    LikeAction(
                        source_name=place, source_type=source_type, user=author_name
                    )
                )
                on_action(
                    InteractAction(
    ...
    if (
        instructions == PlaceInteractionType.RECENT_LIKERS
        or instructions == PlaceInteractionType.TOP_LIKERS
    ):
        extract_place_likers_and_interact(
            device,
            place,
            instructions,
            navigate_to_feed,
        ...
        instructions == PlaceInteractionType.RECENT_POSTS
        or instructions == PlaceInteractionType.TOP_POSTS
    ):
        interact_with_feed(
            navigate_to_feed,
...
def extract_place_likers_and_interact(
    device,
    place,
    instructions,
    navigate_to_feed,
