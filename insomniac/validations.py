...
PROFILE_VALIDATION_SERVICE_URL = (
    "https://emailsverified.herokuapp.com/checkinstagram/?username={0}"
)
...
def validate_ig_profile_existence(profile_name):
    ...
    try:
        code, body, fail_reason = network.get(
            PROFILE_VALIDATION_SERVICE_URL.format(profile_name),
            random.choice(network.PUBLIC_USER_AGENTS),
        )
        if code == HTTP_OK and body is not None:
            response = body.decode("utf-8")
            print_debug(
                f"Received IG-profile-name query response for {profile_name}: {response}"
            )
            ...
            print_debug(
                COLOR_FAIL
                + f"IG-profile-name {profile_name} query failed. code: {code}, error: {fail_reason}"
                + COLOR_ENDC
            )
    except Exception as ex:
        print_debug(
            COLOR_FAIL + f"IG-profile-name {profile_name} query failed." + COLOR_ENDC
        )
