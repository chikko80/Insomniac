...
from insomniac.actions_impl import (
    ...
    sort_followings_by_date,
    iterate_over_my_followings,
    do_unfollow,
    FOLLOW_REGEX,
)
...
def unfollow(
    ...
    def iteration_callback_pre_conditions(
        following_name, following_name_view, follow_status_button_view
    ):
        """
        ...
        if follow_status_button_view is not None:
            ...
            if (
                follow_status_from_row in FOLLOW_REGEX.split("|")
                or following_status == FollowingStatus.UNFOLLOWED
            ):
                print(
                    "Skip @"
                    + following_name
                    + ". Following status: "
                    + FollowingStatus.UNFOLLOWED.name
        ...
        if (
            ...
            if storage.is_profile_follows_me_by_cache(following_name):
                print(
                    "Skip @"
                    + following_name
                    + ". Following status (according to cache): "
    ...
    def iteration_callback(
        ...
        (
            is_unfollow_limit_reached,
            unfollow_reached_source_limit,
            unfollow_reached_session_limit,
...
def get_unfollow_restriction(followed_by_anyone, unfollow_non_followers):
    ...
    unfollow_restriction_matrix = {
        True: {
            True: UnfollowRestriction.ANY_NON_FOLLOWERS,
            False: UnfollowRestriction.ANY,
        },
        False: {
            True: UnfollowRestriction.FOLLOWED_BY_SCRIPT_NON_FOLLOWERS,
