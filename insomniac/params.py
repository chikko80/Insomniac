...
def parse_arguments(all_args_dict, starter_conf_file_path):
    ...
    if args.config_file is not None:
        if not os.path.exists(args.config_file):
            print(
                COLOR_FAIL
                + "Config file {0} could not be found".format(args.config_file)
                + COLOR_ENDC
            )
            parser.print_help()
            return False, None
...
def refresh_args_by_conf_file(args, conf_file_name=None) -> bool:
    ...
    for param in params:
        if param[CONFIG_PARAMETER_ENABLED]:
            args.__setattr__(
                param[CONFIG_PARAMETER_NAME], param[CONFIG_PARAMETER_VALUE]
            )
...
def load_app_id(config_file):
    ...
    for param in params:
        if param.get(CONFIG_PARAMETER_NAME) == PARAMETER_APP_ID and param.get(
            CONFIG_PARAMETER_ENABLED
        ):
            app_id = param.get(CONFIG_PARAMETER_VALUE)
        elif param.get(CONFIG_PARAMETER_NAME) == PARAMETER_DEVICE_ID and param.get(
            CONFIG_PARAMETER_ENABLED
        ):
            device_id = param.get(CONFIG_PARAMETER_VALUE)
        elif param.get(CONFIG_PARAMETER_NAME) == PARAMETER_APP_NAME and param.get(
            CONFIG_PARAMETER_ENABLED
        ):
...
def load_param(config_file, params_name):
    ...
    for param in params:
        if param.get(CONFIG_PARAMETER_NAME) == params_name and param.get(
            CONFIG_PARAMETER_ENABLED
        ):
...
def resolve_app_id(app_id, device_id, app_name):
    if app_name is not None:
        ...
        if app_id_by_name is not None:
            ...
            print(
                COLOR_FAIL
                + f'You provided app name "{app_name}" but there\'s no app with such name'
                + COLOR_ENDC
            )
...
def _load_params(config_file):
    ...
    if not os.path.exists(config_file):
        print(
            COLOR_FAIL
            + "Config file {0} could not be found".format(config_file)
            + COLOR_ENDC
        )
        return None
