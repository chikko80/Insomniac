...
class InteractBySourceActionRunner(CoreActionsRunner):
    ...
    def set_params(self, args):
        ...
        if args.interact is not None:
            ...
            self.interact = [
                source if not is_source_hashtag(source) else ("#" + source)
        ...
        if args.interaction_users_amount is not None:
            if len(self.interact) > 0:
                users_amount = get_value(
                    args.interaction_users_amount, "Interaction user amount {}", 100
                )
    ...
    def run(
        ...
        from insomniac.action_runners.interact.action_handle_blogger import (
            handle_blogger,
            extract_blogger_instructions,
        )
        from insomniac.action_runners.interact.action_handle_hashtag import (
            handle_hashtag,
            extract_hashtag_instructions,
        )
        from insomniac.action_runners.interact.action_handle_place import (
            handle_place,
            extract_place_instructions,
        )
        ...
        for source in self.interact:
            ...
            while not self.action_status.get() == ActionState.DONE:
                job()
                if (
                    self.action_status.get_limit() == ActionState.SOURCE_LIMIT_REACHED
                    or self.action_status.get_limit()
                    == ActionState.SESSION_LIMIT_REACHED
                ):
                    break
...
class InteractByTargetsActionRunner(CoreActionsRunner):
    ...
    ACTION_ARGS = {
        ...
        "posts_urls_list": {
            ...
            "default": [],
            "metavar": (
                "https://www.instagram.com/p/ID-a",
                "https://www.instagram.com/p/ID-b",
            ),
        },
        "likes_count": {
            "help": "number of likes for each interacted user, 2 by default. "
    ...
    def run(
        ...
        while target is not None:
            ...
            print(
                COLOR_BOLD
                + f"Handle {'@' if target_type == TargetType.USERNAME else ''}"
                + target
                + COLOR_ENDC
            )
        ...
        if target is None:
            print(
                "There are no more new targets to interact with in the database (all been already interacted / filtered)."
            )
            print(
                "If you wish to continue interacting with targets, add new targets to the database using scrapping / targets.txt "
