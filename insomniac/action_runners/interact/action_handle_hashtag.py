...
def extract_hashtag_instructions(source):
    ...
    if selected_instruction is None:
        print(
            "Couldn't use interaction-instructions "
...
def handle_hashtag(
    ...
    def interact_with_profile(liker_username, liker_username_view):
        ...
        (
            ...
            LikeAction(
                source_name=hashtag, source_type=source_type, user=liker_username
            ),
            session_state,
        )
    ...
    def navigate_to_feed():
        ...
        if (
            instructions == HashtagInteractionType.RECENT_LIKERS
            or instructions == HashtagInteractionType.RECENT_POSTS
        ):
    ...
    def interact_with_feed_post(posts_views_list):
        (
            ...
            InteractAction(
                source_name=hashtag, source_type=source_type, user=None, succeed=True
            ),
            session_state,
        )
        ...
        (
            ...
            LikeAction(source_name=hashtag, source_type=source_type, user=None),
            session_state,
        )
        ...
        if like_chance > like_percentage:
            ...
            if author_name is not None:
                on_action(
                    LikeAction(
                        source_name=hashtag, source_type=source_type, user=author_name
                    )
                )
                on_action(
                    InteractAction(
    ...
    if (
        instructions == HashtagInteractionType.RECENT_LIKERS
        or instructions == HashtagInteractionType.TOP_LIKERS
    ):
        extract_hashtag_likers_and_interact(
            device,
            hashtag,
            instructions,
            navigate_to_feed,
        ...
        instructions == HashtagInteractionType.RECENT_POSTS
        or instructions == HashtagInteractionType.TOP_POSTS
    ):
        interact_with_feed(
            navigate_to_feed,
