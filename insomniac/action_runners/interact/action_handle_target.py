...
def handle_target(
    ...
    def pre_conditions(target_name, target_name_view):
        if is_passed_filters is not None:
            ...
            should_continue, is_all_filters_satisfied = is_passed_filters(
                device, target_name, reset=True, filters_tags=["BEFORE_PROFILE_CLICK"]
            )
    ...
    def interact_with_username_target(target_name, target_name_view):
        ...
        (
            ...
            LikeAction(source_name=target, source_type=source_type, user=target_name),
            session_state,
        )
        ...
        (
            ...
            FollowAction(source_name=target, source_type=source_type, user=target_name),
            session_state,
        )
        ...
        (
            ...
            StoryWatchAction(
                source_name=target, source_type=source_type, user=target_name
            ),
            session_state,
        )
        ...
        can_follow = (
            ...
            and storage.get_following_status(target_name) == FollowingStatus.NONE
        ...
        if not can_interact:
            ...
            print_interaction_types(
                target_name, can_like, can_follow, can_watch, can_comment
            )
            ...
            is_liked, is_followed, is_watch, is_commented = interaction(
                username=target_name, interaction_strategy=interaction_strategy
            )
            if is_liked or is_followed or is_watch or is_commented:
                ...
                print_short_report(
                    InsomniacSessionState.SOURCE_NAME_TARGETS, session_state
                )
            else:
                on_action(
                    InteractAction(
    ...
    def interact_with_post_id_target(target_post, target_username, target_name_view):
        ...
        if not can_interact:
            ...
            print_interaction_types(
                f"{target_username} - {target_post}", can_like, False, False, False
            )

            opened_post_view.like()
            print(
                COLOR_OKGREEN
                + f"@{target_username} - {target_post} - photo been liked."
                + COLOR_ENDC
            )
            on_action(
                LikeAction(
    ...
    if target_type == TargetType.USERNAME:
        ...
        if is_myself:
            print(
                COLOR_FAIL
                + "Target handling was started for yourself. Abort."
                + COLOR_ENDC
            )
            return
        ...
        if validate_url(url) and "instagram.com/p/" in url:
            if (
                open_instagram_with_url(
                    device_id=device.device_id, app_id=device.app_id, url=url
                )
                is True
            ):
