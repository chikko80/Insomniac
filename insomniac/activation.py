...
class ActivationController:
    ...
    def validate(self, activation_code, ui=False):
        ...
        if not self.is_activated:
            dot = "\n    - "
            print_timeless(
                f"Hi! Since of v3.1.0 all core features in this project are free to use.\n"
    ...
    def get_extra_feature(self, module, ui=False):
        ...
        code, body, fail_reason = network.get(
            f"{HOST}{extra_feature_path}{module}"
        ...
        print(
            COLOR_FAIL
            + f"Cannot get {'ui-' if ui else ''}module {module} from v{__version__}: "
...
def print_activation_required_to(action):
    print_timeless(
        ...
        + COLOR_BOLD
        + f"{HOST}{PATH_ACTIVATE}"
        + COLOR_ENDC
    )


def _validate(activation_code, ui):
    code, _, fail_reason = network.get(
        f"{HOST}{PATH_VALIDATE}?activation_code={activation_code}{'&ui=true' if ui else ''}"
    )
    if code == HTTP_OK:
        print(
            COLOR_OKGREEN + "Your activation code is confirmed, welcome!" + COLOR_ENDC
        )
        return True

    print(
        COLOR_FAIL
        + f"Activation code is not confirmed: {code} ({fail_reason})"
        + COLOR_ENDC
    )
    return False
