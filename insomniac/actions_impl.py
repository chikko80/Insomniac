...
FOLLOWERS_BUTTON_ID_REGEX = (
    "{0}:id/row_profile_header_followers_container"
...
LISTVIEW_OR_RECYCLERVIEW_REGEX = (
...
def scroll_to_bottom(device):
    ...
    def is_end_reached():
        see_all_button = device.find(
            resourceId=f"{device.app_id}:id/see_all_button",
...
def open_user(
    device, username, refresh=False, deep_link_usage_percentage=0, on_action=None
):
    return _open_user(
        device, username, False, False, refresh, deep_link_usage_percentage, on_action
    )


def open_user_followers(
    device, username, refresh=False, deep_link_usage_percentage=0, on_action=None
):
    return _open_user(
        device, username, True, False, refresh, deep_link_usage_percentage, on_action
    )


def open_user_followings(
    device, username, refresh=False, deep_link_usage_percentage=0, on_action=None
):
    return _open_user(
        device, username, False, True, refresh, deep_link_usage_percentage, on_action
    )
...
def iterate_over_likers(device, iteration_callback, iteration_callback_pre_conditions):
    likes_list_view = device.find(
        resourceId="android:id/list", classNameMatches=LISTVIEW_OR_RECYCLERVIEW_REGEX
    )
    ...
    while True:
        ...
        try:
            for item in device.find(
                resourceId=f"{device.app_id}:id/row_user_container_base",
                className="android.widget.LinearLayout",
            ):
                user_name_view = item.child(
                    resourceId=f"{device.app_id}:id/row_user_primary_name",
                    className="android.widget.TextView",
                )
                if not user_name_view.exists(quick=True):
                    print(
                ...
                if not to_continue:
                    print(
                        COLOR_OKBLUE + "Stopping hashtag-likers iteration" + COLOR_ENDC
                    )
                    print(f"Going back")
                    device.back()
                    return False
...
def interact_with_user(
    ...
    if username == my_username:
        print("It's you, skip.")
        return (
            liked_count == interaction_strategy.likes_count,
            is_followed,
            is_watched,
            is_commented,
        )
    ...
    if interaction_strategy.do_story_watch:
        is_watched = _watch_stories(
            device,
            user_source,
            source_type,
            username,
            interaction_strategy.stories_count,
            on_action,
        )
    ...
    def do_like_actions():
        ...
        if interaction_strategy.do_like or interaction_strategy.do_comment:
            ...
            suggestions_container = device.find(
                resourceId=f"{device.app_id}:id/similar_accounts_container",
                className="android.widget.LinearLayout",
            )
            if suggestions_container.exists(quick=True):
                print("Close suggestions to avoid bugs while scrolling")
                arrow_button = device.find(
                    resourceId=f"{device.app_id}:id/row_profile_header_button_chaining",
            ...
            def on_like():
                ...
                print(
                    COLOR_OKGREEN
                    + "@{} - photo been liked.".format(username)
                    + COLOR_ENDC
                )
                on_action(
                    LikeAction(
                        source_name=user_source, source_type=source_type, user=username
                    )
                )
            ...
            for i in range(0, interaction_strategy.likes_count):
                ...
                if not _open_photo_and_like_and_comment(
                    ...
                    row,
                    column,
                    interaction_strategy.do_like,
                    interaction_strategy.do_comment,
                    interaction_strategy.like_percentage,
                    on_like,
                    interaction_strategy.comment_percentage,
                    interaction_strategy.comments_list,
                    my_username,
                    on_comment,
                ):
                    print(
    ...
    def do_follow_action():
        ...
        if interaction_strategy.do_follow:
            is_followed = _follow(
                device,
                username,
                interaction_strategy.follow_percentage,
                is_scrolled_down,
            )
            if is_followed:
                on_action(
    ...
    if interaction_strategy.do_follow and (
        interaction_strategy.do_like or interaction_strategy.do_comment
    ):
...
def _open_photo_and_like_and_comment(
    device,
    row,
    column,
    do_like,
...
def _comment(device, my_username, comments_list, on_comment):
    ...
    if not comment_button.exists(
        quick=True
    ) or not ActionBarView.is_in_interaction_rect(comment_button):
    ...
    post_button = device.find(
        resourceId=f"{device.app_id}:id/layout_comment_thread_post_button_click_area"
    )
    post_button.click()
...
def _follow(device, username, follow_percentage, is_scrolled_down):
    ...
    profile_header_main_layout = device.find(
        resourceId=f"{device.app_id}:id/profile_header_fixed_list",
        className="android.widget.LinearLayout",
    )

    shop_button = profile_header_main_layout.child(
        className="android.widget.Button", clickable=True, textMatches=SHOP_REGEX
    )

    if shop_button.exists(quick=True):
        follow_button = profile_header_main_layout.child(
            className="android.widget.Button", clickable=True, textMatches=FOLLOW_REGEX
        )
        if not follow_button.exists(quick=True):
            print(
                COLOR_FAIL
                + "Look like a shop profile without an option to follow, continue."
                + COLOR_ENDC
            )
            return False
    else:
        profile_header_actions_layout = device.find(
            resourceId=f"{device.app_id}:id/profile_header_actions_top_row",
            className="android.widget.LinearLayout",
        )
        ...
        if not follow_button.exists(quick=True):
            ...
            if unfollow_button.exists(quick=True):
                print(
                    COLOR_OKGREEN + "You already follow @" + username + "." + COLOR_ENDC
                )
                return False
            else:
                print(
...
def do_have_story(device):
    return device.find(
        resourceId=f"{device.app_id}:id/reel_ring", className="android.view.View"
    ).exists(quick=True)
...
def is_already_followed(device):
    ...
    unfollow_button = profile_header_main_layout.child(
        classNameMatches=TEXTVIEW_OR_BUTTON_REGEX,
        clickable=True,
        textMatches=ALREADY_FOLLOWING_REGEX,
    )
...
def _watch_stories(
    device, source_name, source_type, username, stories_value, on_action
):
    if stories_value == 0:
        return False
    ...
    if do_have_story(device):
        ...
        if profile_picture.exists():
            print(
                ...
                + f"Watching @"
                + username
                + f" stories, at most {stories_value}"
                + COLOR_ENDC
            )
            ...
            on_action(
                StoryWatchAction(
                    source_name=source_name, source_type=source_type, user=username
                )
            )
            sleeper.random_sleep()
...
def _is_story_opened(device):
    reel_viewer = device.find(
        resourceId=f"{device.app_id}:id/reel_viewer_root",
...
def iterate_over_my_followers(
    device, iteration_callback, iteration_callback_pre_conditions
):
    _iterate_over_my_followers_or_followings(
        device, iteration_callback, iteration_callback_pre_conditions, is_followers=True
    )


def iterate_over_my_followings(
    device, iteration_callback, iteration_callback_pre_conditions
):
    _iterate_over_my_followers_or_followings(
        device,
...
def _iterate_over_my_followers_or_followings(
    ...
    while True:
        ...
        for item in device.find(
            ...
            to_continue = iteration_callback(
                username, user_name_view, follow_status_button_view
            )
            if to_continue:
                sleeper.random_sleep()
                ...
                print(
                    COLOR_OKBLUE
                    + f"Stopping iteration over {entities_name}"
                    + COLOR_ENDC
                )
                return
        ...
        if screen_iterated_followings > 0:
            ...
            print(
                COLOR_OKGREEN
                + f"No {entities_name} were iterated, finish."
                + COLOR_ENDC
            )
            return
...
def sort_followings_by_date(device, sort_order):
    ...
    sort_button = device.find(
        resourceId=f"{device.app_id}:id/sorting_entry_row_icon",
        className="android.widget.ImageView",
    )
    ...
    if not sort_item.exists():
        print(
            COLOR_FAIL
            + f"Cannot find an option to sort by {sort_order.name}"
            + COLOR_ENDC
        )
        device.back()
        return
...
def do_unfollow(
    ...
    if (
        follow_status_button_view is not None
        and not check_if_is_follower
        and unfollow_from_list_chance > 50
    ):
        ...
        if check_if_is_follower:
            if _check_is_follower(device, username, my_username):
                ...
                storage.update_follow_status(
                    username, is_follow_me=True, do_i_follow_him=True
                )
                print("Back to the followings list.")
                device.back()
                return False
            storage.update_follow_status(
                username, is_follow_me=False, do_i_follow_him=True
            )
...
def open_likers(device):
    ...
    if not posts_view_list.is_visible():
        ...
        if likers_list_view.is_visible():
            print(
                COLOR_FAIL
                + "Oops, likers list is opened instead of posts list. Going back."
                + COLOR_FAIL
            )
            posts_view_list = likers_list_view.press_back_arrow()
        else:
            raise Exception(
                "We are supposed to be on posts list, but something gone wrong."
            )
